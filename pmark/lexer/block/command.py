...
class BlockLexerCommandKind(Enum):
    ...
    """
    Bootstrap the lexer by creating the root frame.

    This command is used *exactly once* per lexer invocation. It is not
    generated by any rule; instead, it is explicitly set as the `causal_command`
    of the root frame during lexer initialization. All subsequent frames are
    created by other commands (e.g., `TOKENIZE_NESTED`).
    """
...
SPECULATIVE_SAFE_COMMAND_KINDS: Final[frozenset[BlockLexerCommandKind]] = frozenset(
    {
        BlockLexerCommandKind.COMMIT_SUCCESS,
...
"""
Set of command kinds that are permitted when the lexer is running in speculative mode.
Speculative mode must not produce tokens or otherwise mutate the token stream;
